...
class TIGERCARE(TIGER):
    """CARE variant for TIGER semantic-ID generation.

    CARE keeps the item semantic IDs unchanged. It replaces the downstream
    decoder target sequence with stage-wise learned query embeddings and uses
    the last query of each stage to predict the corresponding SID token.
    """
    ...
    def __init__(self, config):
        ...
        if len(self.care_query_numbers) < self.care_sid_length:
            self.care_query_numbers.extend([self.care_query_numbers[-1] if self.care_query_numbers else 1] * (
                self.care_sid_length - len(self.care_query_numbers)
            ))
